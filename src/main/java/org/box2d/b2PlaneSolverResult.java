// Generated by jextract

package org.box2d;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet lang=c :
 * struct b2PlaneSolverResult {
 *     struct b2Vec2 translation;
 *     int iterationCount;
 * };
 * }
 */
public class b2PlaneSolverResult {

    public static MemoryLayout $LAYOUT() {
        return constants$98.const$2;
    }
    public static MemorySegment translation$slice(MemorySegment seg) {
        return seg.asSlice(0, 8);
    }
    public static VarHandle iterationCount$VH() {
        return constants$98.const$3;
    }
    /**
     * Getter for field:
     * {@snippet lang=c :
     * int iterationCount;
     * }
     */
    public static int iterationCount$get(MemorySegment seg) {
        return (int)constants$98.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet lang=c :
     * int iterationCount;
     * }
     */
    public static void iterationCount$set(MemorySegment seg, int x) {
        constants$98.const$3.set(seg, x);
    }
    public static int iterationCount$get(MemorySegment seg, long index) {
        return (int)constants$98.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void iterationCount$set(MemorySegment seg, long index, int x) {
        constants$98.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


